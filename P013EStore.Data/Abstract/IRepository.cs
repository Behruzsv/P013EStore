...
namespace P013EStore.Data.Abstract
{
    public interface IRepository<T> where T : class // IRepository interface i dışarıdan alacağı T tipinde bir parametreyle çalışacak ve where şartı ile bu T nin veri tipi bir class olmalıdır dedik.
    {
        ...
        Task<T> GetAsync(Expression<Func<T, bool>> expression); // lambda expression kullanarak db de filtreleme yaparak geriye 1 tane kayıt döndür
        Task<List<T>> GetAllAsync();
        Task<List<T>> GetAllAsync(Expression<Func<T, bool>> expression);// lambda expression kullanarak db de filtreleme yaparak geriye liste döndür
